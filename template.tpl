...

{
  ...
  "description": "Configuration tag that needs to run before any event can be send",
  "containerContexts": [
    "WEB"
  ]
}
...
[
  ...
  },
  {
    ...
    "subParams": [
      ...
      {
        "type": "CHECKBOX",
        "name": "noPageloadEvent",
        "checkboxText": "Disable pageload event",
        "simpleValueType": true,
        "help": "Check this box when you want to manually send the pageload event"
      },
      {
        "type": "TEXT",
        "name": "pageloadEventID",
        "displayName": "Event ID (Only for the pageload event)",
        "simpleValueType": true,
...
scenarios:
- name: succesfull_init
  code: |-
    // Call runCode to run the template's code.
    runCode(mockData);

    // Make sure the queue is created and BillyPix is set window

    // assertApi('createArgumentsQueue').wasCalledWith('StagBillyPix', 'StagBillyPix.queue');
    assertApi('setInWindow').wasCalled();

    // Verify that the tag finished successfully.
    // NOTE: gtm.onsuccess is called asynchronously so can't be checked as this is synchronous
    assertApi('injectScript').wasCalled();
- name: no_tracking_id_set
  code: |-
    // Remove tracking id from input data
    mockData.trackingID = undefined;
...
setup: |-
  // Usefull imports for tests
  const log = require('logToConsole');

  let mockData = {
    "isDebug": true,
    "useStaging": true,
    "trackingID": "ID-XX-XXX",
    "gtmTagId": 2147483646,
    "gtmEventId": 1
  };
...
Created on 25/06/2024, 17:18:56
